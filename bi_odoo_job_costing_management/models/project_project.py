...
class Project(models.Model):
    ...
    issue_ids = fields.One2many('project.issue', 'project_id', string="Issues Ids", domain=['|', ('stage_id.fold', '=', False), ('stage_id', '=', False)])
    ...
    def _get_purchase_count(self):
        ...
        for job_cost_sheet in self:
            ...
            for purchase in project_ids:
                ...
                purchase_ids = self.env['purchase.order'].search([('id', 'in', purchase_line_ids.order_id.ids)])
    ...
    def _compute_issue_count(self):
        for project_id in self:
            ...
            project_id.issue_count = len(issue_ids)
        return
    ...
    def button_view_issue(self):
        ...
        if total_issue:
            project_project = self.env['ir.actions.act_window']._for_xml_id('bi_odoo_job_costing_management.project_issue_categ_act0')
